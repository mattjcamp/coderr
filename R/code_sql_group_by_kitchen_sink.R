...
#' Aggregate Grouped Records By All Possible Combinations
#'
#' SQL Code to get aggregated statistics for all combinations of groups
...
code_sql_kitchen_sink <- function(sql, aggregator.cols, aggregator.cols.with.stat.def, group.by.cols){

  if (!any(!c("sql_sample", "character", "db_table") %in% class(sql)))
      stop("sql_group_by_all_possible_combinations requires sql to be one of these objects: character, sql_sample or db_table")
